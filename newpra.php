...
?>
<div class="pranew_pra_container">
				...
				<table width="100%" align="center" cellpadding="0" cellspacing="0">
					<tr>
						<td style="padding-left:7px">
							<?php
								$q = "SELECT *,product.pic1 as productimg , product.view_num as viewproduct  ,product.name as productname FROM `product`  LEFT JOIN member ON ( member.id = product.shop_id ) WHERE product.prarelease = '2' AND product.active = '2' AND product.score > '80' AND product.warning = '0' AND product.recommend = 0 AND NOT product.certificate = 2 AND member.active='2'";
								// $q = "SELECT * FROM `product` WHERE prarelease = '2' AND active = '2' AND score > '80' AND warning = '0' AND recommend = 0 AND NOT certificate = 2 ";
								$p_r = 1;
								$db->query($q);
								$total = $db->num_rows();
							...
							?>	            
							<table width="148" style="float:left; margin:3px; width:148px;" border="0" cellpadding="0" cellspacing="0">
					<tr>
						<td align="center">
								...
								<tr>
									<td>
										<div style="position:relative;">
											<div class="pravote_container" style="display: none; position: absolute; left: 187px; top: -183px;">
		...
		<table align="right" border="0" cellpadding="0" cellspacing="0">
			<tr>
				<?php
					...
					if($_POST["page_min"] > 0){
				?>
				<td page_num="<?=$page_num - 1?>" page_total="" onclick="page_select($(this),$('.pranew_pra_container'),'newpra.php','<?=$page_min?>','<?=$page_max?>');" style="white-space:nowrap; cursor:pointer;">
					previous
				</td>
				<?php
					}
				...
				<td style="white-space:nowrap;">
					...
					<table style="float:left; margin-left:10px; height:20px; cursor:pointer;" border="0" cellpadding="0" cellspacing="0">
						<tr>
							<td page_num="<?=$l?>" page_total="" onclick="page_select($(this),$('.pranew_pra_container'),'newpra.php','<?=$page_min?>','<?=$page_max?>');" style="text-align:center; <?php if($l == $page_num){ echo "color:#e00000;"; }else{ echo "color:#ffffff;"; } ?> ">
								<?=$l?>
							</td>
						</tr>
					</table>
				...
				?>
				<td page_num="<?=$page_num + 1?>" page_total="<?=$page_total?>" onclick="page_select($(this),$('.pranew_pra_container'),'newpra.php','<?=$page_min?>','<?=$page_max?>');" style="padding-left:10px; white-space:nowrap; cursor:pointer;">
					next
				</td>
				<?php
					}
