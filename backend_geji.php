		...
		<table width="1000" border="0" align="center" cellpadding="0" cellspacing="0" id="Table_01">
			...
			<tr>
				<td height="826" valign="top" style="padding:5px; background-color:#311308 ">
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
						<tr>
							<td width="19%" valign="top" style="border-right:1px solid #000">
								<table width="100%" border="0" cellspacing="0" cellpadding="0">
									<tr>
										<td>
							...
							<td width="81%" valign="top">
								...
								<?php 
									if($_POST['Submit']){
										if($_POST['h_geji_id']==''){
												...
												$q="INSERT INTO `geji` ( `geji_id` , `topic` , `detail` , `mem_id` , `pic1`)  VALUES (  '', '".$_POST['topic']."' , '".$_POST['detail']."' , '".$_SESSION['adminshop_id']."' , '".$file_up2."');";
												$db->query($q);
										al('Add Complete');
										redi2();
										}else{
												$q="UPDATE `geji` SET `topic` = '".$_POST['topic']."' ,
												`detail` = '".$_POST['detail']."' WHERE `geji_id` =".$_POST['h_geji_id']." ";
												$db->query($q);
												if($_FILES['file']['name']!=''){
													@unlink("img/geji/".$_POST['h_pic']);
													upimg2($_FILES['file'],"img/geji/");    
													$q="UPDATE `geji` SET `pic1` = '".$file_up2."' WHERE `geji_id` =".$_POST['h_geji_id']." ";
													$db->query($q);
												}
									al('Edit Complete');
									redi2();              
								...
								<?php
									if($_GET['d_geji_id']){
										...
										$q="DELETE FROM `geji` WHERE `geji_id` =".$_GET['d_geji_id']." ";
										$db->query($q);       
									}
									?>
								<?php
									if($_GET['e_geji_id']){
										$q="SELECT * FROM geji WHERE geji_id=".$_GET['e_geji_id']." ";
										$db5=new nDB();
										$db5->query($q);
										$db5->next_record();
									}
								...
								<form action="" method="post" enctype="multipart/form-data" name="form1" id="form1">
									<table width="95%" border="0" align="center" cellpadding="0" cellspacing="2" class="box_from3_2" style="color:#FC0">
										<tr>
										...
										<tr>
											<td height="25" colspan="2" align="center" bgcolor="#4d1403" class="bh">ประวัติพระเกจิ / 编辑高僧介绍区</td>
										</tr>
										<tr>
											<td width="26%" align="right" bgcolor="#3c1204" class="sidemenu"  style="padding-right:3px; color:#FC0">ชื่อหัวข้อ / 标题</td>
											<td width="74%" bgcolor="#3c1204"><input name="topic" type="text" class="box_from3" id="topic" value="<?=($_GET['e_geji_id'])?$db5->f(topic):""?>" size="45" /></td>
										</tr>
										<tr>
											<td align="right" bgcolor="#3c1204" class="sidemenu"  style="padding-right:3px; color:#FC0">รายละเอียด / 详容</td>
											<td bgcolor="#3c1204">
										...
										<tr>
											...
											<td align="center" valign="top" bgcolor="#3c1204" class="sidemenu" >            <?php if($_GET['e_geji_id']){ ?>
												<img src="/img/geji/<?=$db5->f(pic1)?>" width="100" height="100" />
												<br />            
												<?php } ?>            <input name="file" type="file"  />
											</td>
										</tr>
										<tr>
								...
								<table width="700" border="0" align="center" cellpadding="0" cellspacing="3" bordercolor="#FFFFFF">
									<tr class="bh">
										<td width="17%" height="25" align="center" bgcolor="#4d1403" class="bh" >&nbsp;</td>
										<td width="63%" height="25" align="center" bgcolor="#4d1403" style="color:#FC0">ชื่อหัวข้อ <span class="sidemenu" style="padding-right:3px">/ 标题</span></td>
									...
									<?php 
										$q="SELECT * FROM `geji` WHERE mem_id = '".$_SESSION['adminshop_id']."' ORDER BY geji_id DESC";
										$db->query($q);
										static $v=1;
										while($db->next_record()){
										?>
									<tr bgcolor="<?=($v%2==0)?"#F8F8F8":"#FFFFFF"?>">
										<td height="110" align="left" bgcolor="#3c1204" style="padding-left:10px" ><img src="/resize/w100-h100/img/geji/<?=$db->f(pic1)?>" width="100" height="100" /></td>
										<td align="left" bgcolor="#3c1204"  style="color:#FC0"><?=$db->f(topic)?></td>
										<td width="10%" align="center" bgcolor="#3c1204"><a href="?e_geji_id=<?=$db->f(geji_id)?>" ><img src="../images/edit.gif" alt="แก้ไข" width="19" height="23" border="0" /></a></td>
										<td width="10%" align="center" bgcolor="#3c1204" ><a href="?d_geji_id=<?=$db->f(geji_id)?>" onClick="return confirm('Confirm Delete?')" ><img src="../images/del.gif" alt="ลบ" width="19" height="23" border="0" /></a></td>
									</tr>
									<?php $v++; } ?>
								</table>
							</td>
