...
<div class="prarecommanded_container">
...
<?php
	$q = "SELECT *,product.pic1 as productimg , product.view_num as viewproduct  ,product.name as productname FROM `product` LEFT JOIN member ON ( member.id = product.shop_id ) WHERE recommend = '1' AND product.active = '2' AND product.score > '80' AND warning = '0' AND member.active='2' ";
	// $q = "SELECT * FROM `product` WHERE recommend = '1' AND active = '2' AND score > '80' AND warning = '0' ";
	$p_r = 1;
	$db->query($q);
	$total = $db->num_rows();
	$q .= " ORDER BY hotdate DESC";
	if($_POST["page_min"] == ""){
		$q .= " LIMIT 0,24";
	}else{
		...
		<table align="right" border="0" cellpadding="0" cellspacing="0">
			<tr>
				<?php
					...
					if($_POST["page_min"] > 0){
				?>
				<td page_num="<?=$page_num - 1?>" page_total="" onclick="page_select($(this),$('.prarecommanded_container'),'recommendpra.php','<?=$page_min?>','<?=$page_max?>');" style="white-space:nowrap; cursor:pointer;">
					previous
				</td>
				<?php
					}
				...
				<td style="white-space:nowrap;">
					...
					<table style="float:left; margin-left:10px; height:20px; cursor:pointer;" border="0" cellpadding="0" cellspacing="0">
						<tr>
							<td page_num="<?=$l?>" page_total="" onclick="page_select($(this),$('.prarecommanded_container'),'recommendpra.php','<?=$page_min?>','<?=$page_max?>');" style="text-align:center; <?php if($l == $page_num){ echo "color:#e00000;"; }else{ echo "color:#ffffff;"; } ?> ">
								<?=$l?>
							</td>
						</tr>
					</table>
				...
				?>
				<td page_num="<?=$page_num + 1?>" page_total="<?=$page_total?>" onclick="page_select($(this),$('.prarecommanded_container'),'recommendpra.php','<?=$page_min?>','<?=$page_max?>');" style="padding-left:10px; white-space:nowrap; cursor:pointer;">
					next
				</td>
				<?php
					}
