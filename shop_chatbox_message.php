...
<script>
	...
	function chatbox_refresh(){
		$.ajax({
			type: "POST",
			url: "shop_chatbox_query.php",
			data: { do_what:"chatbox_refresh", shop_id:"<?=$_GET['shop']?>" },
			cache: false,
			success: function(result){
	...
	function chatbox_message(message){
		...
		$.ajax({
			type: "POST",
			url: "shop_chatbox_query.php",
			data: { do_what:"chatbox_message", message:message, shop_id:"<?=$_GET['shop']?>" },
			cache: false,
			success: function(result){
				chatbox_refresh();
			}
	...
	function chatbox_emoticon(emoticon){
		$.ajax({
			type: "POST",
			url: "shop_chatbox_query.php",
			data: { do_what:"chatbox_emoticon", emoticon:emoticon, shop_id:"<?=$_GET['shop']?>" },
			cache: false,
			success: function(result){
				chatbox_refresh();
			}
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0">
		...
		<tr>
			<td class="chatbox_tools">
				...
				<input name="shop_id" value="<?=$_GET["shop"]?>" type="hidden"/>
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
					<tr>
									...
									<td>
										...
										<div style="position:relative;">
											...
											<div style="position:absolute; opacity:0; left:0px; top:0px; width:25px; height:25px; overflow:hidden;">
												<form action="shop_chatbox_query.php" target="chatbox_iframe" method="post" enctype="multipart/form-data">
				...
				<table style="margin-bottom:5px;" width="100%" border="0" cellpadding="0" cellspacing="0">
					<tr>
						<td width="715" style="padding-left:5px; width:50%;">
							<table width="100%" border="0" cellpadding="0" cellspacing="0">
								<tr>
									<td class="chatbox_title_sub">
										แปลภาษา / 翻译
						...
						<td width="705" style="padding-left:5px; width:50%;">
							<table width="100%" border="0" cellpadding="0" cellspacing="0">
								<tr>
									<td class="chatbox_title_sub">
										ฝากแปลภาษา / 存录为了帮我翻译
						...
						<td width="109" style="padding-left:5px; width:1px;">
							<input onclick="chatbox_translate_leave();" class="button_flat_orange" style="height:50px; word-break:break-all;" value="ฝากแปล / 存录为了帮我翻译" type="button"/>
						</td>
					</tr>
				</table>
				<?php
