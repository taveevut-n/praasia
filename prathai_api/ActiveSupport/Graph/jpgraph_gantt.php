...
class GanttScale {
	...
	function StrokeWeeks($aYCoord, $getHeight = false) {
		if ($this->week->iShowLabels) {
			...
			} elseif ($this->week->iStyle == WEEKSTYLE_FIRSTDAY || $this->week->iStyle == WEEKSTYLE_FIRSTDAY2 || $this->week->iStyle == WEEKSTYLE_FIRSTDAYWNBR || $this->week->iStyle == WEEKSTYLE_FIRSTDAY2WNBR) {
			...
			for($i = 0; $i < $this->GetNumberOfDays () / 7; ++ $i, $x += $weekwidth) {
				...
				elseif ($this->week->iStyle == WEEKSTYLE_FIRSTDAY || $this->week->iStyle == WEEKSTYLE_FIRSTDAYWNBR)
					$txt = date ( "j/n", $week );
				elseif ($this->week->iStyle == WEEKSTYLE_FIRSTDAY2 || $this->week->iStyle == WEEKSTYLE_FIRSTDAY2WNBR) {
	...
	function StrokeMonths($aYCoord, $getHeight = false) {
		if ($this->month->iShowLabels) {
			...
			$img->SetTextAlign ( "center" );
			if ($this->GetMonthNbr ( $this->iStartDate ) == $this->GetMonthNbr ( $this->iEndDate ) && $this->GetYear ( $this->iStartDate ) == $this->GetYear ( $this->iEndDate )) {
...
class LinkArrow {
	var $ix, $iy;
	var $isizespec = array (array (2, 3 ), array (3, 5 ), array (3, 8 ), array (6, 15 ), array (8, 22 ) );
