...
final class ColumnSetModel {
	...
	public function addColumn(ColumnModel $oCM) {
		...
		$this->_columns [] = $this->columns [$oCM->getFieldName ()];
		unset ( $oCM );
	}
...
final class RecordModel {
	...
	public function setMetaRecord(ColumnSetModel $oCol) {
		$this->mMetaRecord = $oCol;
	}
	public function cloneMetaRecord() {
		return $this->mMetaRecord;
	}
...
abstract class DataModel {
	 ...
	 *
	 * @param array $k
	 * @example [Object]->setBelong(array('table_name'=>'tbl_example','on'=>'pk_id'));
	 * @deprecated
	 *
	 *
	 *
	 ...
	 *
	 * @param array $k
	 * @example [Object]->relateTo(array('path_config'=>'configdb.xml','table_name'=>'tbl_example','pk'=>'id'));
	 */
	 ...
	 * @throws Exception
	 *
	 * @example [Object]->relateTo(DataModel $dm [,array("from_map_field"=>"to_map_field")]);
	 */
	public function addRelative(DataModel $dm, $s = null, $_OnKey = array()) {
		if ($s == null) {
			$s = $dm->getTable ();
		}

		if (count ( $_OnKey ) == 0 || is_string ( $_OnKey )) {
			throw new Exception ( __CLASS__ . __METHOD__ . " : Please, Input array Type or not blank array" );
		}

                //if($this->$s->mParent != $dm){
                    $this->$s = $dm;
...
//                echo "<br/>---------<br/>";
	...
	private function getListProperty() {
		...
		if ($o_db->intState == 1) {
			...
			for($_i_cm = 0; $_i_cm < $o_db->countField (); $_i_cm ++) {
				...
				for($j = 0; $j < count ( $this->mPK ); $j ++) {
					if ($this->mPK [$j] == $keyName) {
						$_cm->setPK ( true );
	...
	/**
	 * ส่งชื่อสำหรับสร้างฟังก์ชันกลับมาให้
	 *
	 * @param string $s
	 ...
	 * @return boolean
	 */
	public function deleteRelateParent() {
		$b_ = false;
		$w = "";

		if ($w == "" || $w == null) {
			for($j = 0; $j < count ( $this->mParent->mPK ); $j ++) {
				if ($j != 0) {
					$w .= " and ";
				}
				$w .= $this->mParent->mPK [$j] . " = '" . $this->mParent->getValue ( $this->mParent->mPK [$j] ) . "'";
			}
		}

		$sql = "DELETE FROM {$this->mTable} WHERE {$w};";
		if ($this->mOnKey != null) {
	 ...
	 */
	public function findRelateParent($order_by = array()) {
		$this->mRecordSet->freeRecords ();
		$this->freeCursor ();
		...
		$j = 0;
		foreach ( $this->mOnKey as $k => $v ) {
			if ($j != 0) {
				$w .= " and ";
			}

			$w .= $k . " = '" . $this->mParent->getValue ( $v ) . "'";
			$j ++;
		}
		...
		$strSql = "SELECT * FROM {$this->mTable} WHERE {$w} {$s};";
	...
	public function findRelate() {
		...
		for($j = 0; $j < count ( $this->mPK ); $j ++) {
			...
			$w .= $this->mPK [$j] . " = '" . $this->mParent->getValue ( $this->mParent->mPK [$j] ) . "'";
		}

		echo $strSql = "SELECT * FROM {$this->mTable} WHERE {$w};";

		if ($this->mOnKey != null) {
			echo $this->mOnKey;
			echo "</br/>";
			echo $strSql = "SELECT * FROM {$this->mTable} WHERE $this->mPK = '" . $this->mParent->getValue ( $this->mOnKey ) . "';";
		}
	 ...
	 * @return nubmer
	 */
	public function getFieldSize($k) {
		return $this->mRecordSet->getMetaRecord ()->getColumnByIndex ( $k )->getFieldSize ();
	}
	 ...
	 * @return array
	 */
	public function getPkFieldName($i_key = 0) {
		return $this->mPK [$i_key];
	}
