			...
			<td height="56" align="center" style="background:#FC3 url(images/tab.jpg) no-repeat; border-top:3px solid #960; font-size:16px">
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
					<tr>
						...
						<td style="text-align:center;color:#7b3110; font-weight:bold">สินค้าเบ็ดเตล็ด 杂项区</td>
					</tr>
				</table>
			</td>
		</tr>
				...
				<table width="100%" align="center" cellpadding="0" cellspacing="0">
					<tr>
						<td style="padding-left:7px">
							<?php
								$q="SELECT *,product.pic1 as productimg , product.view_num as viewproduct ,product.name as productname   FROM `product` LEFT JOIN member ON ( member.id = product.shop_id ) WHERE product.other = '1' AND product.active = '2' AND product.score > '80' AND product.warning = '0' AND product.recommend = 0 AND product.certificate = 0 AND member.active ='2'";
								// $q="SELECT * FROM `product` WHERE other = '1' AND active = '2' AND score > '80' AND warning = '0' AND recommend = 0 AND certificate = 0 ";
								$p_r = 1;
								$db->query($q);
								$total = $db->num_rows();
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="3">
							<tr>
								<td style="text-align: right;">
									<span style="color:#FFF">
										<img src="images/view-icon.png" width="20" height="11" /> <?=$db->f(viewproduct)?>
									</span>
								</td>
							</tr>
						</table>
		...
		<table align="right" border="0" cellpadding="0" cellspacing="0">
			<tr>
				<?php
					...
					if($_POST["page_min"] > 0){
				?>
				<td page_num="<?=$page_num - 1?>" page_total="" onclick="page_select($(this),$('.praother_container'),'otherpra.php','<?=$page_min?>','<?=$page_max?>');" style="white-space:nowrap; cursor:pointer;">
					previous
				</td>
				<?php
					}
				...
				<td style="white-space:nowrap;">
					...
					<table style="float:left; margin-left:10px; height:20px; cursor:pointer;" border="0" cellpadding="0" cellspacing="0">
						<tr>
							<td page_num="<?=$l?>" page_total="" onclick="page_select($(this),$('.praother_container'),'otherpra.php','<?=$page_min?>','<?=$page_max?>');" style="text-align:center; <?php if($l == $page_num){ echo "color:#e00000;"; }else{ echo "color:#ffffff;"; } ?> ">
								<?=$l?>
							</td>
						</tr>
					</table>
				...
				?>
				<td page_num="<?=$page_num + 1?>" page_total="<?=$page_total?>" onclick="page_select($(this),$('.praother_container'),'otherpra.php','<?=$page_min?>','<?=$page_max?>');" style="padding-left:10px; white-space:nowrap; cursor:pointer;">
					next
				</td>
				<?php
					}
