	...
	<tr>
		<td style="background-color:#4f3b2a;">
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="0">
				...
				<?php
					$q_message = mysql_query("select * from twe_message msg left join member m on msg.sender_id = m.id where sender_id = '".$_SESSION["adminshop_id"]."' order by msg.message_id desc");
					$index = 0;
					while($message = mysql_fetch_array($q_message)){
				?>
				<tr class="pm_tr" onclick="$('.pmdetail_tr').hide();$(this).next().show();" style="height:30px;">
					<td style="padding-left:20px; padding-right:20px; white-space:nowrap;" onclick="window.location.href='shop_index.php?shop=<?=$message["receiver_id"]?>'">
						<?=$message["name"]?>
					</td>
