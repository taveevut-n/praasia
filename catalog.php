...
`province_id` = '".$_POST['province']."' WHERE `catalog_id` =".$_POST['h_sub_id']." ";
		$db->query($q);
		$q="UPDATE `product` SET `name_catalog` = '".$_POST['h_sub_id']."' WHERE name_catalog='".$_POST['name_catalog2']."' ";
		$db->query($q);			
		al('แก้ไขข้อมูลเรียบร้อยแล้ว');
		redi2();
		}else{				
		$q="INSERT INTO `catalog` ( `catalog_id` ,`catalog_name` , `province_id`) 
VALUES (
'', '".$_POST['name_catalog2']."', '".$_POST['province']."'
);";
		$db->query($q);
		...
		if(mdr.value==000){
			...
			mdr.value=0;
				  ...
				  <script>
                    ...
                    <tr>
                      <td align="right" bgcolor="#3c1204" class="style11" style="padding-right:3px"><span class="sidemenu">ภาค :</span></td>
