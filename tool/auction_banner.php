...
<?php 
	if($_POST['Submit']){
		if($_POST['h_banner_id']==''){
				...
				$q="INSERT INTO `pramool_banner` ( `banner_id` , `banner_url` , `banner_detail` , `banner_img` , `banner_pos` ) 	VALUES (	'', '".$_POST['banner_url']."' , '".$_POST['banner_detail']."' , '".$file_up2."' , '".$_POST['pos']."' );";
				$db->query($q);
		al('Add Complete');
		redi2();
		}else{
				$q="UPDATE `pramool_banner` SET `banner_url` = '".$_POST['banner_url']."' ,
				...
				if($_FILES['file']['name']!=''){
					...
					upimg2($_FILES['file'],"../img/banner/");		
					$q="UPDATE `pramool_banner` SET `banner_img` = '".$file_up2."' WHERE `banner_id` =".$_POST['h_banner_id']." ";
					$db->query($q);
				}
	al('Edit Complete');
	redi2();							
...
<?php
	if($_GET['d_banner_id']){
		...
		$q="DELETE FROM `pramool_banner` WHERE `banner_id` =".$_GET['d_banner_id']." ";
		$db->query($q);				
	}
?>
                  <?php
	if($_GET['e_banner_id']){
		$q="SELECT * FROM pramool_banner WHERE banner_id=".$_GET['e_banner_id']." ";
		$db5=new nDB();
		$db5->query($q);
		$db5->next_record();
	}
...
<form action="" method="post" enctype="multipart/form-data" name="form1" id="form1">
				   ...
				   <table width="700" border="0" align="center" cellpadding="0" cellspacing="3" bordercolor="#FFFFFF">
						  ...
						  <?php 
						  	$q="SELECT * FROM `pramool_banner` WHERE 1 ORDER BY banner_id DESC";
							$db->query($q);
							static $v=1;
