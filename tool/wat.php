...
<?php 
	if($_POST['Submit']){
		if($_POST['h_geji_id']==''){
				upimg2($_FILES['file'],"../img/wat/");		
				$q="INSERT INTO `wat` ( `wat_id` , `topic` , `detail` , `pic1`) 	VALUES (	'', '".$_POST['topic']."' , '".$_POST['detail']."' , '".$file_up2."');";
				$db->query($q);
		al('Add Complete');
		redi2();
		}else{
				$q="UPDATE `wat` SET `topic` = '".$_POST['topic']."' ,
				`detail` = '".$_POST['detail']."' WHERE `wat_id` =".$_POST['h_geji_id']." ";
				$db->query($q);
				if($_FILES['file']['name']!=''){
					@unlink("../img/wat/".$_POST['h_pic']);
					upimg2($_FILES['file'],"../img/wat/");		
					$q="UPDATE `wat` SET `pic1` = '".$file_up2."' WHERE `wat_id` =".$_POST['h_geji_id']." ";
					$db->query($q);
				}
	al('Edit Complete');
	redi2();							
...
<?php
	if($_GET['d_geji_id']){
		@unlink("../img/wat/".$_GET['d_pic']);
		$q="DELETE FROM `wat` WHERE `wat_id` =".$_GET['d_geji_id']." ";
		$db->query($q);				
	}
?>
                  <?php
	if($_GET['e_geji_id']){
		$q="SELECT * FROM wat WHERE geji_id=".$_GET['e_geji_id']." ";
		$db5=new nDB();
		$db5->query($q);
		$db5->next_record();
	}
...
<form action="" method="post" enctype="multipart/form-data" name="form1" id="form1">
                    <table width="95%" border="0" align="center" cellpadding="0" cellspacing="2" class="box_from3_2">
                      ...
                      <tr>
						...
						<img src="../img/wat/<?=$db5->f(pic)?>" width="274" height="100" />
						<br />						
						<?php } ?>
						รูปภาพ 
                          <input name="file" type="file"  /></td>
                            ...
                            <tr>
                              ...
                              <td bgcolor="#3c1204"><input name="Submit" type="submit" class="button_add" value="<?=($_GET['e_geji_id'])?"แก้ไขข้อมูล":"เพิ่มข้อมูล"?>" />
                                  ...
                                  <input name="h_geji_id" type="hidden" id="h_geji_id" value="<?=$db5->f(wat_id)?>" />
				   ...
				   <table width="700" border="0" align="center" cellpadding="0" cellspacing="3" bordercolor="#FFFFFF">
						  ...
						  <?php 
						  	$q="SELECT * FROM `wat` WHERE 1 ORDER BY wat_id DESC";
							$db->query($q);
							static $v=1;
							while($db->next_record()){
						  ?>
                          <tr bgcolor="<?=($v%2==0)?"#F8F8F8":"#FFFFFF"?>">
                            <td height="110" align="left" bgcolor="#3c1204" style="padding-left:10px" ><img src="../slir/w100-h100/img/wat/<?=$db->f(pic1)?>" width="100" height="100" /></td>
                            <td align="center" bgcolor="#3c1204" class="sidemenu" ><?=$db->f(topic)?></td>
                            <td width="8%" align="center" bgcolor="#3c1204"><a href="?e_geji_id=<?=$db->f(wat_id)?>" ><img src="../images/edit.gif" alt="แก้ไข" width="19" height="23" border="0" /></a></td>
                            <td width="8%" align="center" bgcolor="#3c1204" ><a href="?d_geji_id=<?=$db->f(wat_id)?>" onClick="return confirm('Confirm Delete?')" ><img src="../images/del.gif" alt="ลบ" width="19" height="23" border="0" /></a></td>
                     </tr>
						  <?php $v++; } ?>
      </table>                  
                  </td>
