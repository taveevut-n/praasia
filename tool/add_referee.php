...
<form action="" method="post" enctype="multipart/form-data" name="form1" id="form1" target="frm">
                    <table width="95%" border="0" align="center" cellpadding="0" cellspacing="2" class="box_from3_2">
                            <tr>
                              <td height="25" colspan="2" align="center" bgcolor="#4d1403" class="bh">เพิ่ม<span class="style11">ที่ปรึกษา</span>ตรวจสอบพระ</td>
                      </tr>
							 <tr>
                              <td width="20%" height="31" align="right" bgcolor="#3c1204" class="sidemenu"  style="padding-right:3px">Username</td>
                              <td width="80%" bgcolor="#3c1204" style="color:#FFF"><input name="username" type="text" class="box_from3" id="username" value="<?=($_GET['e_data_id'])?$db5->f(username):""?>" size="80" /><br />ที่ปรึกษาเป็นคนนอกไม่ต้องใส่</td>
                              </tr>
							 <tr>
							 ...
							 <tr>
                              <td width="20%" height="31" align="right" bgcolor="#3c1204" class="sidemenu"  style="padding-right:3px">URL</td>
                              <td width="80%" bgcolor="#3c1204"><input name="url" type="text" class="box_from3" id="username" value="<?=($_GET['e_data_id'])?$db5->f(url):""?>" size="80" /></td>
                              </tr>
							 <tr>
                              <td width="20%" height="31" align="right" bgcolor="#3c1204" class="sidemenu"  style="padding-right:3px">บันทึกข้อความ</td>
                              <td width="80%" bgcolor="#3c1204"><textarea name="remark" class="box_from3" id="remark" style="height:100px; width:400px"><?=($_GET['e_data_id'])?$db5->f(remark):""?>
                              </textarea></td>
                      </tr>          
							 <tr>
				...
				<iframe name="frm" width="1" height="1" frameborder="0"></iframe>				   <br />
				   <table width="700" border="0" align="center" cellpadding="0" cellspacing="3" bordercolor="#FFFFFF">
							<?
						  	$q="SELECT * FROM `datacert` WHERE datacert_id = '".$_SESSION['amulet_id']."' ";
							$dbamulet=new nDB();
							$dbamulet->query($q);
							$dbamulet->next_record();
							?>                   
                         <tr class="bh">
                            <td height="25" align="center" bgcolor="#000000" class="style11" colspan="5" > ที่ปรึกษาการตรวจสอบพระ <?=$dbamulet->f(datacert_amulet)?></td>
                     </tr> 
                          <tr class="bh">
						  ...
						  <?php 
						  	$q="SELECT * FROM `referee` WHERE amulet_id = '".$_SESSION['amulet_id']."' ORDER BY referee_id DESC";
							$db->query($q);
							static $v=1;
                          ...
                          <tr bgcolor="<?=($v%2==0)?"#3c1204":"#3c1204"?>">
                          	<td>
                            <a href="<? if ($db->f(username)=='') { ?><?=$db->f(url)?><? } else { ?>../shop_index.php?shop=<?=$dbmember->f(id)?><? } ?>" target="_blank">
                            <img src="<?=($db->f(username)=='')?'../img/referee_pic/'.$db->f(pic1):'../img/head/'.$dbmember->f(head2)?>" width="100" height="100" />
                            </a>
                            </td>
