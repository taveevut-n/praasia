...
<?php 
	if($_POST['Submit']){
				$q="UPDATE `pra_order` SET 
				`detail` = '".$_POST['detail']."' WHERE `id` =".$_POST['e_front_id']." ";
				$db->query($q);
		redi4('pra_order.php?e_front_id='.$_GET['e_front_id']);							
		}			
?>

                  <?php
	if($_GET['e_front_id']){
		$q="SELECT * FROM pra_order WHERE id=".$_GET['e_front_id']." ";
		$db5=new nDB();
		$db5->query($q);
		$db5->next_record();
	}
...
<form action="" method="post" enctype="multipart/form-data" name="form1" id="form1">
              <table width="95%" border="0" align="center" cellpadding="0" cellspacing="2" class="box_from3_2">
                            <tr>
                              <td height="25" colspan="2" align="center" bgcolor="#4d1403" class="bh">วิธีการสั่งซื้อสินค้า</td>
                      </tr>                        
			    <tr>
                              <td align="right" bgcolor="#3c1204" class="sidemenu"  style="padding-right:3px">Detail</td>
                              <td bgcolor="#3c1204">
							                                <textarea name="detail" cols="60" rows="5" id="detail" style="overflow:hidden"><?=$db5->f(detail)?></textarea>                
							  <script>
