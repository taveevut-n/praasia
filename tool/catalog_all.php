...
`top_id` = '".$_POST['top_catalog']."' WHERE `catalog_id` =".$_POST['h_sub_id']." ";
		...
		$q="INSERT INTO `catalog_all` ( `catalog_id` ,`top_id`,`catalog_name`,`catalog_name_cn`) 
VALUES (
...
<?php
	if($_GET['d_sub_id']){	
		$q="DELETE FROM `catalog_all` WHERE `catalog_id` = ".$_GET['d_sub_id']." ";
		$db->query($q);
	}
?>
<?php
	if($_GET['e_sub_id']){
		$q="SELECT * FROM `catalog_all` WHERE catalog_id=".$_GET['e_sub_id']." ";
		$db5 = new nDB();
		$db5->query($q);
		$db5->next_record();
	}
