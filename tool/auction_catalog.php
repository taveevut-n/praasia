				...
				<script language="javascript">
					function selec(){
						...
						location.href=ab.value;
							...
							<?php
	if($_GET['sub_id']){
		$q="update `pramool_catalog` set show_sub='".$_GET['status']."' WHERE `id_sub_catalog` =".$_GET['sub_id']." ";
		$db->query($q);
		redi2();				
	}
?>	
							<?php
	if($_GET['status_id']){
		$q="update `pramool_catalog` set status='".$_GET['status']."' WHERE `id_sub_catalog` =".$_GET['status_id']." ";
		$db->query($q);
		redi2();				
	}
?>	
...
`top_id` = '".$_POST['top_catalog']."' WHERE `catalog_id` =".$_POST['h_sub_id']." ";
		$db->query($q);
		$q="UPDATE `pramool_product` SET `catalog_id` = '".$_POST['name_catalog2']."' WHERE catalog_id='".$_POST['h_sub_id']."' ";
		$db->query($q);			
		al('แก้ไขข้อมูลเรียบร้อยแล้ว');
		redi2();
		}else{				
		$q="INSERT INTO `pramool_catalog` ( `catalog_id` ,`top_id`,`catalog_name`,`catalog_name_cn`) 
VALUES (
...
<?php
	if($_GET['d_sub_id']){	
		$q="DELETE FROM `pramool_catalog` WHERE `catalog_id` = ".$_GET['d_sub_id']." ";
		$db->query($q);
	}
?>
<?php
	if($_GET['e_sub_id']){
		$q="SELECT * FROM `pramool_catalog` WHERE catalog_id=".$_GET['e_sub_id']." ";
		$db5 = new nDB();
		$db5->query($q);
		$db5->next_record();
	}
?>
				  ...
				  <script>
                    ...
                    <tr>
                      ...
                      <td width="73%" bgcolor="#3c1204"><span class="sidemenu">
						...
						<?
						$q="SELECT * FROM `pramool_catalog` WHERE top_id = 0 ORDER BY catalog_id  ";
						$db_top=new nDB();
						$db_top->query($q);
  ...
  ?>
		...
		$q1="SELECT * FROM `pramool_catalog` WHERE top_id = '".$db->f(catalog_id)."' ORDER BY catalog_id  ";
		$db5=new nDB();
		$db5->query($q1);
		static $v=1;
		if($db5->num_rows()!=0){
