				...
				<script language="javascript">
					function selec(){
						...
						location.href=ab.value;
							...
							<?php
	if($_GET['sub_id']){
		$q="update `pra_catalog2` set show_sub='".$_GET['status']."' WHERE `id_sub_catalog` =".$_GET['sub_id']." ";
		$db->query($q);
		redi2();				
	}
?>	
							<?php
	if($_GET['status_id']){
		$q="update `pra_catalog2` set status='".$_GET['status']."' WHERE `id_sub_catalog` =".$_GET['status_id']." ";
		$db->query($q);
		redi2();				
	}
?>	
	...
	if($_POST['Submit']){
		if($_POST['h_sub_id']){
		$q="UPDATE `pra_catalog2` SET `id_top_catalog` = '".$_POST['id_sub_catalog']."',
`status` = '".$_POST['status']."',		
`name_catalog` = '".$_POST['name_catalog']."',
`detail_catalog` = '".$_POST['detail_catalog']."' WHERE `id_sub_catalog` =".$_POST['h_sub_id']." ";
		$db->query($q);
		$q="UPDATE `pra_product2` SET `name_catalog` = '".$_POST['name_catalog']."' WHERE name_catalog='".$_POST['h_name_catalog']."' ";
		$db->query($q);		
				if($_FILES['file']['name']!=''){
					@unlink("../upimg/cat/".$_POST['h_pic']);
					upimg2($_FILES['file'],"../upimg/cat/");		
					$q="UPDATE `pra_catalog2` SET `img_cat` = '".$file_up2."' WHERE `id_sub_catalog` =".$_POST['h_sub_id']." ";
					$db->query($q);
				}		
		...
		$q="INSERT INTO `pra_catalog2` ( `id_sub_catalog` , `id_top_catalog` , `name_catalog`, `detail_catalog`, `img_catalog`, `status` ) 
VALUES (
...
<?php
	if($_GET['d_sub_id']){
		...
		$q="DELETE FROM `pra_catalog2` WHERE `id_sub_catalog` = ".$_GET['d_sub_id']." ";
		$db->query($q);
	}
?>
<?php
	if($_GET['e_sub_id']){
		$q="SELECT * FROM `pra_catalog2` WHERE id_sub_catalog=".$_GET['e_sub_id']." ";
		$db5 = new nDB();
		$db5->query($q);
		$db5->next_record();
	}
?>
	...
	function dr(){
		...
		if(mdr.value==000){
			...
			mdr.value=0;
				  ...
				  <table width="96%" border="0" align="center" cellpadding="0" cellspacing="1">
                    <tr class="sidemenu">
                      <td width="73%" height="25" align="center" bgcolor="#4d1403" class="style11" >ชื่อหมวดสินค้า</td>
                      <td align="center" bgcolor="#4d1403" class="style11" >แสดงหน้าแรก</td>
  ...
  ?>
                    <tr>
                      <td bgcolor="#1c0801" style="padding-left:3px"><span style="color:#FC0; font-size:12px"><?=$db->f(name_catalog)?></span></td>
                      <td bgcolor="#1c0801">&nbsp;</td>
                      <td width="8%" align="center" bgcolor="#1c0801"><a href="?e_sub_id=<?=$db->f(id_sub_catalog)?>" ><img src="images/edit.gif" alt="แก้ไข" width="19" height="23" border="0" /></a></td>
                      <td width="8%" align="center" bgcolor="#1c0801" ><a href="?d_sub_id=<?=$db->f(id_sub_catalog)?>"  onclick="return confirm('ยืนยันการลบข้อมูล')"><img src="images/del.gif" alt="ลบ" width="19" height="23" border="0" /></a></td>
                    </tr>
                    <?php  
...
?>
                    <tr  bgcolor="<?=($v%2==1)?"#3c1204":"#2f0d02"?>">
                      <td class="sidemenu" >--- <?=$db5->f(name_catalog)?></td>
                      <td align="center"><?php
					if($db5->f(status)=='0'){
				?>
				<a href="?status_id=<?=$db5->f(id_sub_catalog)?>&status=1" >
					<img src="../images/nohot.gif" alt="Disable" width="16" height="16" border="0">
				</a>
				<?php
					}
					if($db5->f(status)=='1'){
				?>
				<a href="?status_id=<?=$db5->f(id_sub_catalog)?>&status=0" >
					<img src="../images/hot.gif" alt="Pause" width="16" height="16" border="0">
				</a>
				<?php }?></td>
                      <td width="8%" align="center" ><a href="?e_sub_id=<?=$db5->f(id_sub_catalog)?>" ><img src="images/edit.gif" alt="แก้ไข" width="19" height="23" border="0" /></a></td>
                      <td width="8%" align="center" ><a href="?d_sub_id=<?=$db5->f(id_sub_catalog)?>"  onclick="return confirm('ยืนยันการลบข้อมูล')"><img src="images/del.gif" alt="ลบ" width="19" height="23" border="0" /></a></td>
                    </tr>
                    <?php $v++; ?>
                    <?php } } }  ?>
                  </table>
