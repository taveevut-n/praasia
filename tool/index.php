						...
						<tr>
							...
							<td valign="top" bgcolor="#3f1d0e">
								...
								<table width="95%" border="1" align="center" cellpadding="3" cellspacing="0" bordercolor="#2a1100" style="border-collapse:collapse">
									<tr class="bh">
										<td width="23%" height="30" align="center" bgcolor="#5f2206">รหัสสมาชิก</td>
										...
										<td align="center" bgcolor="#5f2206">Backlist</td>
									...
									<?php
										...
										if($_GET['status']){
											$q = "update `member` set `confirmed` ='".$_GET['status']."' WHERE `id` = '".$_GET['id']."' ";
											$db->query($q);
											redi2();
										}
											...
											if ($_GET['active']=='2') {
													$q = "update `member` set active='".$_GET['active']."' WHERE `id` =".$_GET['id']." ";
													$db->query($q);		
											...
											if ($_GET['blacklist']=='0'){
													$q = "update `member` set backlist ='0' WHERE `id` =".$_GET['id']." ";
													$db->query($q);												
													$q = "update `product` set active ='1' WHERE `shop_id` =".$_GET['id']." ";
													$db->query($q);
													redi2();
												}
											if ($_GET['blacklist']=='1'){
													$q = "update `member` set backlist ='1' WHERE `id` =".$_GET['id']." ";
													$db->query($q);												
													$q = "update `product` set active ='2' WHERE `shop_id` =".$_GET['id']." ";
													$db->query($q);		
													redi2();											
										...
										// if(isset($_GET['d_shop_id'])){
										// 	$d_shop_id = trim($_GET['d_shop_id']);

										// 	mysql_query("DELETE FROM `member` WHERE `id` = $d_shop_id ");
										// 	mysql_query("DELETE FROM `member_payment` WHERE `mem_id` = $d_shop_id ");
										// 	mysql_query("DELETE FROM `twe_message` WHERE `sender_id` = $d_shop_id ");
										// 	mysql_query("DELETE FROM `twe_message` WHERE `receiver_id` = $d_shop_id ");

										// 	$resutlt = mysql_query("SELECT * FROM `product` WHERE shop_id = $d_shop_id ");
										// 	while ($r = mysql_fetch_array($resutlt)) {
										// 		@unlink("img/amulet/".$r['pic1']);
										// 	}	

										// 	mysql_query("DELETE FROM `product` WHERE shop_id = $d_shop_id ");

										// }

										if(isset($_GET['backlist'])){
											$backlist = trim($_GET['backlist']);

											$textBl = mysql_fetch_array(mysql_query("SELECT * FROM `member` WHERE `id` = $backlist AND backlist IS NULL "));
											if(empty($textBl)){
												$q = "UPDATE `member` SET backlist = '1'  WHERE `id` = $backlist ";
												$q = "UPDATE `product` SET active = '0'  WHERE `shop_id` = $backlist ";
											}else{
												$q = "UPDATE `member` SET backlist = '0'  WHERE `id` = $backlist ";
												$q = "UPDATE `product` SET active = '2'  WHERE `shop_id` = $backlist ";
											}
										...
										$q = "SELECT * FROM `member` WHERE type = '0' AND NOT id= '1' ORDER BY id DESC";
										$db->query($q);
										static $v = 1;
									...
									<tr class="sidemenu" bgcolor="<?=($v%2==0)?"#3c1204":"#2f0d02"?>">
										...
										<td height="25" align="center">
											<a style="color:#ffffff;line-height: 20px;" href="http://www.praasia.com/shop_index.php?shop=<?=$db->f(id)?>" target="_blank">
											<?=$db->f(shopname)?>
											</a>
										</td>
										<td align="center" style="line-height: 20px;"><?=$db->f(username)?></td>
										<td align="center" style="line-height: 20px;"><?=$db->f(name)?></td>
										<td width="1" align="center" style="line-height: 20px;">
											<?php
												if($db->f(backlist) ==1){?>
											<a href="?id=<?=$db->f(id)?>&blacklist=0" >
												<img src="../images/play.png" alt="Disable" width="24" height="24" border="0">
											</a>
												<?php }else  if($db->f(backlist) == 0) { ?>
											<a href="?id=<?=$db->f(id)?>&blacklist=1" >
												<img src="../images/stop.png" alt="Enable" width="24" height="24" border="0">
											</a>
												<?php
												}
												?>
										...
										<td align="center">
											<?php
												...
												?>
											<a href="?fileno_id=<?=$db->f(id)?>&status=2" >
											<img src="../images/stop.png" alt="Disable" width="24" height="24" border="0">
											</a>
											<?php
												}
												...
												?>
											<a href="?fileno_id=<?=$db->f(id)?>&status=0" >
											<img src="../images/play.png" alt="Enable" width="24" height="24" border="0">
											</a>
											<?php
												}
												...
												?>
											<a href="?fileno_id=<?=$db->f(id)?>&status=1" >
