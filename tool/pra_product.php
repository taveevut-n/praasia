...
<td width="720" valign="top" bgcolor="#592D03"><table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
	...
	if($_POST['Submit']){
		...
		$q="UPDATE `pra_product` SET `name_catalog` = '".$_POST['name_catalog']."',
...
`price` = '".$_POST['price']."' WHERE `id_product` =".$_POST['h_product_id']." ";
		$db->query($q);
				for($mf=1;$mf<=5;$mf++){
				 $upf[$mf]=uppic($_FILES['file'.$mf],$mf,"../upimg/product/",$_POST['h_pic'.$mf]); // Same folder
					 if($upf[$mf]!=''){
				 $q="UPDATE `pra_product` SET `pic$mf` = '".$upf[$mf]."' WHERE `id_product` =".$_POST['h_product_id']." ";
						 $db->query($q);
					 }	
				}			
		...
		$q="INSERT INTO `pra_product` ( `id_product` , `name_catalog` , `code_product` , `tag_product` , `name_product` , `detail_product` , `price` , `pic1` , `pic2`, `pic3` , `pic4`, `pic5`, `status`,`view_num`) 
VALUES (
'', '".$_POST['name_catalog']."', '".$_POST['code_product']."', '".$_POST['tag_product']."', '".$_POST['name_product']."', '".$_POST['detail_product']."', '".$_POST['price']."', '' , '' , '' , '' , '' , '".$_POST['status']."','0') ";
		$db->query($q);
				for($mf=1;$mf<=5;$mf++){
				 $upf[$mf]=uppic($_FILES['file'.$mf],$mf,"../upimg/product/",$_POST['h_pic'.$mf]); // Same folder
					 if($upf[$mf]!=''){
				$q="SELECT * FROM `pra_product`ORDER BY id_product DESC";
				$db->query($q);
				$db->next_record();	 
				$id_product=$db->f(id_product);
				 $q="UPDATE `pra_product` SET `pic$mf` = '".$upf[$mf]."' WHERE `id_product` =".$id_product." ";
						 $db->query($q);
					 }				
			}
...
<?php
	if($_GET['d_product_id']){
		...
		$q="DELETE FROM `pra_product` WHERE `id_product` = ".$_GET['d_product_id']." ";
		$db->query($q);		
	}
?>
<?php
	if($_GET['e_product_id']){
		$q="SELECT * FROM `pra_product` WHERE id_product=".$_GET['e_product_id']." ";
		$db5=new nDB();
		$db5->query($q);
		$db5->next_record();
	}
?>
                  <form action="" method="post" enctype="multipart/form-data" name="form1" id="form1">
					...
					>---
                      ...
                      <tr>
                        <td align="right" bgcolor="#3c1204" class="style11"  style="padding-right:3px"><span class="sidemenu">Code Product</span></td>
...
<tr>
     ...
     <?php
   $q="SELECT * FROM `pra_product` WHERE 1 ";
   $p_r=1;
   if($_SESSION['pro_catalog']){
   ...
   ?>		
                      <tr  bgcolor="<?=($v%2==1)?"#3c1204":"#2f0d02"?>">
                        ...
                        <td width="7%" align="center" >
							<a href="?e_product_id=<?=$db->f(id_product)?><?php if($_GET["s_page"] != ""){echo "&s_page=".$_GET["s_page"];} ?>" >
