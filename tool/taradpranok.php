...
<?php 
	if($_POST['Submit']){
		if($_POST['h_banner_id']==''){
				...
				$q="INSERT INTO `taradpranok` ( `banner_id` , `banner_url` , `banner_detail` , `banner_img` , `banner_pos` ) 	VALUES (	'', '".$_POST['banner_url']."' , '".$_POST['banner_detail']."' , '".$file_up2."' , '".$_POST['pos']."' );";
				$db->query($q);
		al('Add Complete');
		redi2();
		}else{
				$q="UPDATE `taradpranok` SET `banner_url` = '".$_POST['banner_url']."' ,
				...
				if($_FILES['file']['name']!=''){
					...
					upimg2($_FILES['file'],"../img/banner/");		
					$q="UPDATE `taradpranok` SET `banner_img` = '".$file_up2."' WHERE `banner_id` =".$_POST['h_banner_id']." ";
					$db->query($q);
				}
	al('Edit Complete');
	redi2();							
...
<?php
	if($_GET['d_banner_id']){
		...
		$q="DELETE FROM `taradpranok` WHERE `banner_id` =".$_GET['d_banner_id']." ";
		$db->query($q);				
	}
?>
                  <?php
	if($_GET['e_banner_id']){
		$q="SELECT * FROM taradpranok WHERE banner_id=".$_GET['e_banner_id']." ";
		$db5=new nDB();
		$db5->query($q);
		$db5->next_record();
	}
...
<form action="" method="post" enctype="multipart/form-data" name="form1" id="form1">
                    <table width="95%" border="0" align="center" cellpadding="0" cellspacing="2" class="box_from3_2">
                            <tr>
                              <td height="25" colspan="2" align="center" bgcolor="#4d1403" class="bh">ตลาดพระเครื่องต่างประเทศ</td>
                      </tr>
							 <tr>
							 ...
							 <tr>
                              ...
                              <td bgcolor="#3c1204">
							  <input name="banner_detail" type="text" class="box_from3" id="banner_detail" value="<?=($_GET['e_banner_id'])?$db5->f(banner_detail):""?>" size="45" />        
                              </td>
                      </tr>
                      <tr>
				   ...
				   <table width="700" border="0" align="center" cellpadding="0" cellspacing="3" bordercolor="#FFFFFF">
                          <tr class="bh">
                            <td height="25" align="center" bgcolor="#4d1403" class="style11" >Pic</td>
