...
<?php
	...
	if($do_what == "delProduct"){
		$q="delete from `product` WHERE `product_id` =".$_GET['id']." ";
		$db->query($q);
	}
	?>
<script>
	...
	function redirec_pagechange(){
		...
		page_select(x_this,$('.praold_container'),'list_allproduct.php',page_min,page_max);
	}
...
<div class="praold_container">
	<table width="95%" border="0" align="center" cellpadding="3" cellspacing="1">
		<tr class="bh">
			...
			<td width="79" align="center" bgcolor="#692908">สินค้าสมาชิก</td>
		...
		<tr bgcolor="<?=($v%2==1)?"#311407":"#4f3b2a"?>" class="sidemenu">
			...
			<td align="center">
				...
				<a href="javascript:void(0);" onclick="update_status(<?=$db->f(product_id)?>,1,'certificate_id');"><img src="../images/wait.png" alt="No Hot" width="24" height="24" border="0"></a>
				<? }else{?>
				<a href="javascript:void(0);" onclick="update_status(<?=$db->f(product_id)?>,0,'certificate_id');">
				<img src="../images/ok.png" alt="Hot" width="24" height="24" border="0"></a>
				<? }?>			
			</td>
			<td align="center">
				<? if($db->f(recommend)=='0'){?>
				<a href="javascript:void(0);" onclick="update_status(<?=$db->f(product_id)?>,1,'recommend_id');" ><img src="../images/wait.png" alt="No Hot" width="24" height="24" border="0"></a>
				<? }else{?>
				<a href="javascript:void(0);" onclick="update_status(<?=$db->f(product_id)?>,0,'recommend_id');">
				<img src="../images/ok.png" alt="Hot" width="24" height="24" border="0"></a>
				<? }?>
			</td>
			<td align="center">
				<? if($db->f(showtype)=='0'){?>
				<a href="javascript:void(0);" onclick="update_status(<?=$db->f(product_id)?>,1,'show_id');"><img src="../images/wait.png" alt="No Hot" width="24" height="24" border="0"></a>
				<? }else{?>
				<a href="javascript:void(0);" onclick="update_status(<?=$db->f(product_id)?>,0,'show_id');">
				<img src="../images/ok.png" alt="Hot" width="24" height="24" border="0"></a>
				<? }?>
			</td>
			<td align="center"><a href="javascript:void(0);"  onclick="delProduct(<?=$db->f(product_id)?>);"><img src="images/del.gif" width="19" height="23" border="0"></a></td>
		</tr>
		<?php $v++; } ?>
		<tr>
				...
				<table align="right" border="0" cellpadding="0" cellspacing="0">
					<tr>
						<?php
							...
							if($_GET["page_min"] > 0){
							?>
						<td page_num="<?=$page_num - 1?>" page_total="" onclick="page_select($(this),$('.praold_container'),'list_allproduct.php','<?=$page_min?>','<?=$page_max?>');" style="white-space:nowrap; cursor:pointer;">
							previous
						</td>
						<?php
							}
						...
						<td style="white-space:nowrap;">
							...
							<table style="float:left; margin-left:10px; height:20px; cursor:pointer;" border="0" cellpadding="0" cellspacing="0">
								<tr>
									<td page_num="<?=$l?>" page_total="" onclick="page_select($(this),$('.praold_container'),'list_allproduct.php','<?=$page_min?>','<?=$page_max?>');" style="text-align:center; <?php if($l == $page_num){ echo "color:#e00000;"; }else{ echo "color:#ffffff;"; } ?> ">
										<?=$l?>
									</td>
								</tr>
							</table>
						...
						<?php
							...
							?>
						<td page_num="<?=$page_num + 1?>" page_total="<?=$page_total?>" onclick="page_select($(this),$('.praold_container'),'list_allproduct.php','<?=$page_min?>','<?=$page_max?>');" style="padding-left:10px; white-space:nowrap; cursor:pointer;">
							next
						</td>
						<?php
							}
