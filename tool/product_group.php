...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
					...
					<tr>
							...
							<tr>
								<td align="right">
									<span class="sidemenu">
										...
										<select name="catalog_val_main" onchange="get_CategoryData(this.value,'')" style="min-width:200px;height: 21px;">
											<option value="0">---เลือกหมวดหมู่---</option>
											<?php
												$qgroup = mysql_query("select * from twe_category_main WHERE active = 2 order by order_num asc");
												while ( $r = mysql_fetch_array($qgroup)) {
													?>
													<option value="<?php echo $r['main_id'];?>"><?php echo $r['main_name']."/".$r['main_name_cn'];?></option>
														<?php
													}
											?>
										</select>
						...
						<table width="95%" border="0" align="center" cellpadding="3" cellspacing="1">
							...
							<?php
								...
								if($_GET['q']){
									$name = "and name like '%".$_GET['q']."%' ";
								}
								$q="SELECT * FROM `member` WHERE id='".$_SESSION['shop_id']."' ";
								$db->query($q);
								$db->next_record();
								$shop_id = $db->f(id);			   
								$q="SELECT * FROM `product` WHERE 1 $name";
								$db->query($q);		
								...
								$q.=" ORDER BY product_id DESC LIMIT $page_Start,$per_page ";
								$db->query($q);
							...
							<tr bgcolor="<?=($v%2==1)?"#311407":"#4f3b2a"?>" class="sidemenu">
								...
								<td align="center" width="1" style="position:relative;">
									<div class="box-checkbox">
										<input type="checkbox" name="item[]" value="<?=$db->f(product_id)?>">
									</div>
								...
								<td align="left" width="150"><?=$db->f(name)?></td>
								<td align="center">
									<?php
									$qCat = mysql_query("select * from twe_category where catalog_id in (".$db->f(group_category_id).") ");
									while ($rCat = mysql_fetch_array($qCat)) {
										?>
									<div class="box-tag">
										<?=$rCat['catalog_name'];?>
										<a href="javascript:void(0);" onclick="del_cat($(this),<?php echo $rCat['catalog_id'];?>,<?=$db->f(product_id)?>);">x</a>
									</div>
										<?php
									}
									?>
		...
		<script type="text/javascript">
			function get_CategoryData(a, b) {
			    $.ajax({
			        url: "../backend_add_query.php",
			        type: "POST",
			        dataType: "JSON",
			        data: {
			            do_what: "get_CategoryData",
			            val: a,
			            lang: b
			        },
			        cache: !1,
			        processData: !0,
			        success: function(a) {
			            $("select[name=catalog_val]").empty();
			            var b = '<option value="0">--\u0e40\u0e25\u0e37\u0e2d\u0e01\u0e2b\u0e21\u0e27\u0e14\u0e2b\u0e21\u0e39\u0e48\u0e22\u0e48\u0e2d\u0e22 / \u9009\u62e9\u603b\u7ad9\u4f5b\u724c\u5206\u7c7b\u533a--</option>';
			            $.each(a.rows, function(a, c) {
			                b += '<option value="' + c.catalog_id + '">' + c.catalog_name + "</option>";
			            })

			            $("select[name=catalog_val]").append(b);
			        }
			    })
			}
		</script>
