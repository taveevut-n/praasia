...
<?php 
	if($_POST['Submit']){
		if($_POST['h_banner_id']==''){
				upimg2($_FILES['file'],"../jewelry/upimg/banner/");		
				$q="INSERT INTO `jew_banner` ( `banner_id` , `banner_url` , `banner_detail` , `banner_img` , `banner_pos` ) 	VALUES (	'', '".$_POST['banner_url']."' , '".$_POST['banner_detail']."' , '".$file_up2."' , '".$_POST['pos']."' );";
				$db->query($q);
		al('Add Complete');
		redi2();
		}else{
				$q="UPDATE `jew_banner` SET `banner_url` = '".$_POST['banner_url']."' ,
				`banner_detail` = '".$_POST['banner_detail']."',
				`banner_pos` = '".$_POST['pos']."' WHERE `banner_id` =".$_POST['h_banner_id']." ";
				$db->query($q);
				if($_FILES['file']['name']!=''){
					@unlink("../img/banner/".$_POST['h_pic']);
					upimg2($_FILES['file'],"../jewelry/img/banner/");		
					$q="UPDATE `jew_banner` SET `banner_img` = '".$file_up2."' WHERE `banner_id` =".$_POST['h_banner_id']." ";
					$db->query($q);
				}
	al('Edit Complete');
	redi2();							
...
<?php
	if($_GET['d_banner_id']){
		@unlink("../jewelry/upimg/banner/".$_GET['d_pic']);
		$q="DELETE FROM `jew_banner` WHERE `banner_id` =".$_GET['d_banner_id']." ";
		$db->query($q);				
	}
?>
                  <?php
	if($_GET['e_banner_id']){
		$q="SELECT * FROM jew_banner WHERE banner_id=".$_GET['e_banner_id']." ";
		$db5=new nDB();
		$db5->query($q);
		$db5->next_record();
	}
